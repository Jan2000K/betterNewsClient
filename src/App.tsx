...
function App() {
    ...
    useEffect(
        () => {
            ...
            if (language === "ENG") {
                ...
                let blockedWords = JSON.parse(localStorage.getItem("ENGblockedWordsJSON")!)
                if(blockedWords===null){
                    localStorage.setItem("ENGblockedWordsJSON","[]")
                    blockedWords = JSON.parse(localStorage.getItem("ENGblockedWordsJSON")!)
                }

                let blockedFeeds = JSON.parse(localStorage.getItem("ENGblockedFeedsJSON")!)

                if(blockedFeeds==null){
                    localStorage.setItem("ENGblockedFeedsJSON","[]")
                    blockedFeeds = JSON.parse(localStorage.getItem("ENGblockedFeedsJSON")!)
                }



                axios.all([axios.get(url), axios.post(url, {
                ...
                let blockedWords = JSON.parse(localStorage.getItem("SLOblockedWordsJSON")!)
                let blockedFeeds = JSON.parse(localStorage.getItem("SLOblockedFeedsJSON")!)

                if(blockedWords===null){
                    localStorage.setItem("SLOblockedWordsJSON","[]")
                    blockedWords=JSON.parse(localStorage.getItem("SLOblockedWordsJSON")!)
                }

                if(blockedFeeds==null){
                    localStorage.setItem("SLOblockedFeedsJSON","[]")
                    blockedFeeds=JSON.parse(localStorage.getItem("SLOblockedFeedsJSON")!)
                }
    ...
    useEffect(
        () => {
            if (newsFeed !== undefined && newsFeed !== null) {
                ...
                }else if(language==="ENG" && activeCategory===newsCategory.Slovenia){
    ...
    if (loading) {
        ...
        return (
            <languageContext.Provider value={{value: language, setter: setLanguage}}>
                <Header/>
                <UserFilters activeCategory={{setter: setActiveCategory, value: activeCategory}}
                             newsSources={{setter: setNewsSources, value: newsSources}}
                             filtersChange={{setter: setUserFilterChange, value: userFilterChange}}
                />
